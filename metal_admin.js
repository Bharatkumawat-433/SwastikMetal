...
function renderTable(data) { // Renders table WITH Action buttons
    ...
    data.forEach(record => {
        ...
        row.insertCell().textContent = record.userName; row.insertCell().textContent = record.metalName;
...
function handleEdit(event) { // Populates form for editing
    ...
    const userName = row.cells[0].textContent; const metalName = row.cells[1].textContent;
    const price = row.cells[2].textContent; const city = row.cells[3].textContent;
    recordIdInput.value = recordId; userNameInput.value = userName; metalNameInput.value = metalName;
